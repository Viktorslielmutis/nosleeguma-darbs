...
$welcome_message = ($is_admin) ? "Admin" : $_SESSION["username"];

// Set the SQL query based on user role
$sql = ($is_admin) ? "SELECT product_id, virsraksts, teksts, cena, img_url FROM products" : "SELECT product_id, virsraksts, teksts, cena, img_url FROM products WHERE user_id = ?";

// Prepare the SQL statement
if ($stmt = mysqli_prepare($conn, $sql)) {
    // Bind the user ID parameter if the user is not an admin
    if (!$is_admin) {
                        ...
                        <tbody>
                            <?php if (!empty($products)): ?>
                                <?php foreach ($products as $product): ?>
                                    <tr>
                                        <td><?php echo htmlspecialchars($product['virsraksts']); ?></td>
                                        <td><?php echo htmlspecialchars($product['teksts']); ?></td>
                                        <td><?php echo htmlspecialchars($product['cena']); ?>€</td>
                                        <td>
                                            <div class="resize-profilemid">
                                                <img class="resize-profile" src="<?php echo htmlspecialchars($product['img_url']); ?>" alt="Product Image">
                                            </div>
                                        </td>
                                        ...
                                        <td class="profile-delete">
                                            <a href="delete.php?id=<?php echo $product['product_id']; ?>" onclick="return confirm('Vai tiešām vēlaties dzēst šo produktu?')">
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="6">Produkti nav atrasti.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
