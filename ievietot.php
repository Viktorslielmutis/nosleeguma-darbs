...
if (isset($_POST['submit'])) {
    ...
    if ($check === false) {
        $errorMessages[] = "Sis fails nav bilde.";
        $uploadOk = 0;
    }

    // Check file size (adjust as needed)
    if ($_FILES["Img_URL"]["size"] > 1000000) {
        $errorMessages[] = "Sis fails ir par lielu.";
        $uploadOk = 0;
    }
    ...
    if (!in_array($imageFileType, $allowedFormats)) {
        $errorMessages[] = "Tikai JPG, JPEG, PNG, un GIF formaati ir atlauti.";
        $uploadOk = 0;
    }

    // Check if $uploadOk is set to 0 by an error
    if ($uploadOk == 0) {
        // Upload failed, display error messages
        ...
        if (move_uploaded_file($_FILES["Img_URL"]["tmp_name"], $target_file)) {
            ...
            if ($stmt->execute()) {
                ...
                displayError("Error inserting product: " . $stmt->error);
            }
            $stmt->close();
        } else {
